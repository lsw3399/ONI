    ...
    internal static class SatellitePatcher
    {
        ...
        internal static void ConfigureBuildingDef(BuildingDef def)
        {
            ...
            try
            {
                ...
                // Force main element on the BuildingDef itself where supported.
                // ONI versions differ on member names/types, so set several candidates.
                TrySetBuildingDefPrimaryElementUnobtanium(def);
        ...
        private static void TrySetBuildingDefPrimaryElementUnobtanium(BuildingDef def)
        {
            if (def == null)
                return;
            ...
            try
            {
                // Common definitions use SimHashes for base element.
                TrySetMemberValue(def, "BaseElement", SimHashes.Unobtanium);
                TrySetMemberValue(def, "baseElement", SimHashes.Unobtanium);

                // Some builds use numeric ids rather than SimHashes.
                TrySetMemberValue(def, "ElementID", (int)SimHashes.Unobtanium);
                TrySetMemberValue(def, "elementID", (int)SimHashes.Unobtanium);
            }
            catch
            {
            ...
            foreach (var pe in primaryElements)
            {
                ...
                try
                {
                    ...
                    foreach (var m in methods)
                    {
                        ...
                        if (ps == null || (ps.Length != 1 && ps.Length != 2)) continue;

                        var paramType = ps[0].ParameterType;
                        bool hasAddTagsFlag = ps.Length == 2 && ps[1].ParameterType == typeof(bool);

                        // Most common case: SetElement(SimHashes)
                        if (paramType == typeof(SimHashes))
                        {
                            if (hasAddTagsFlag)
                                m.Invoke(pe, new object[] { SimHashes.Unobtanium, true });
                            else
                        ...
                        {
                            if (hasAddTagsFlag)
                                m.Invoke(pe, new object[] { (int)SimHashes.Unobtanium, true });
                            else
                                m.Invoke(pe, new object[] { (int)SimHashes.Unobtanium });
                            break;
                        }

                        // Some builds expose SetElement(Element) or SetElement(Tag)
                        var unobtaniumElement = ElementLoader.FindElementByHash(SimHashes.Unobtanium);
                        if (unobtaniumElement != null)
                        {
                            if (paramType == typeof(Element))
                            {
                                if (hasAddTagsFlag)
                                    m.Invoke(pe, new object[] { unobtaniumElement, true });
                                else
                                    m.Invoke(pe, new object[] { unobtaniumElement });
                                break;
                            }

                            if (paramType == typeof(Tag))
                            {
                                if (hasAddTagsFlag)
                                    m.Invoke(pe, new object[] { unobtaniumElement.tag, true });
                                else
                                    m.Invoke(pe, new object[] { unobtaniumElement.tag });
                                break;
                            }
                        }
                        ...
                        if (paramType.IsEnum)
                        {
                            ...
                            if (hasAddTagsFlag)
                                m.Invoke(pe, new object[] { enumVal, true });
                            else
                ...
                // 2) Fallback: set cached element references where available.
                try
                {
                    var flags = BindingFlags.Instance | BindingFlags.Public | BindingFlags.NonPublic;
                    var unobtaniumElement = ElementLoader.FindElementByHash(SimHashes.Unobtanium);
                    if (unobtaniumElement != null)
                    {
                        var elementField = pe.GetType().GetField("element", flags) ?? pe.GetType().GetField("Element", flags);
                        if (elementField != null && elementField.FieldType == typeof(Element))
                            elementField.SetValue(pe, unobtaniumElement);

                        var elementProp = pe.GetType().GetProperty("Element", flags);
                        if (elementProp != null && elementProp.CanWrite && elementProp.PropertyType == typeof(Element))
                            elementProp.SetValue(pe, unobtaniumElement, null);
                    }
                }
                catch
                {
                ...
                // 3) Fallback: try to set element id members directly
                try
                {
